    ...
    for i, chunk in enumerate(retrieved_chunks, 1):
        source = chunk['metadata'].get('filename', 
                                     chunk['metadata'].get('Source_Filename', 
                                                         chunk.get('filename', 'غير محدد')))
        
        context_parts.append(
            f"المقطع المرجعي {i} من {source_type} - الملف: {source}\n"
...
1.  **المصادر المرجعية**: استخرج الإجابة فقط من المقاطع المرجعية (أدلة أو قرارات) مع تصحيح أي أخطاء كتابية إن وجدت.
2.  **تنسيق الإجابة المطلوب**:
    يجب أن تكون الإجابة بالتنسيق التالي بدقة:
    المصدر الأول:
    اسم المصدر: [اسم الدليل أو القرار]
    شرح للمذكور في المصدر: [شرح تفصيلي للمحتوى والمعلومات الموجودة]
    رابط المصدر: [الرابط إن وجد، أو "غير متوفر"]
    [وهكذا لجميع المصادر]
3.  **ترتيب المصادر**: ابدأ بالأدلة ثم القرارات، ورتبهم حسب الأهمية.
4.  **الشرح التفصيلي**: في قسم "شرح للمذكور في المصدر" اشرح بالتفصيل:
    *   المحتوى الرئيسي للمصدر
    *   النقاط المهمة والتعليمات
    *   كيف يجيب على السؤال المطروح
5.  **غياب المعلومات**: إذا لم تحتوِ المقاطع المرجعية على إجابة كافية، اكتب:
    > "المستندات المتاحة لا تحتوي على معلومات كافية للإجابة على هذا السؤال"
6.  **اللغة**: استخدم اللغة العربية فقط.
7.  **التفصيل**: اشرح كل مصدر على حدة.
8.  **التنسيق**: استخدم تنسيقاً جذاباً وواضحاً لسهولة القراءة."""
